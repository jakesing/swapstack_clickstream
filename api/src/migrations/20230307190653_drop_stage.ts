...
const parentTableName = `${config.environment}_clickstream_events_parent`;
const tableName = `${config.environment}_clickstream_events_child`;

export async function up(knex: Knex): Promise<void> {
  return knex.schema.dropTableIfExists(tableName).then(() => {
    return knex.schema.dropTableIfExists(parentTableName);
  });
}
